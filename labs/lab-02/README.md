...
В качестве улучшения рекомендую настроить passive-interface default с явным исключением только интерфейсов, где ожидаются OSPF-соседи - это лучшая практика безопасности и контроля протокола. По диагностике: для полноты картины стоит показывать show ip ospf database (проверка LSDB), а также диагностику на лифах (соседства, таблицу маршрутизации, пинги) - это подтвердит, что маршрутная информация распространилась симметрично. ECMP в вашей топологии должен работать на лифах до удалённых лифов - хорошо бы это наглядно продемонстрировать.